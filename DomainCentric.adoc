...
- Synchronous reading: applicative facade - reading only ! no write !
- Logical sharing: NEVER

Application using Asynchronous events can be split into microservices communicating with kafka for example. Regarding facade - synchronous - it can be replaced by a rest api and rest client.

Event should be used to propage a state using eventual consistency.
